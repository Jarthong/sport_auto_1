...
normal_activity_id = GetData('activity_id', 'activity_lottery_winning_record',
                             "where user_id='{userid}'".format(userid=GlobalVar.GVar['user_id'])).result()
max_activity_id = GetData('max(activity_id)', 'activity_lottery_winning_record').result() + 1
min_activity_id = GetData('min(activity_id)', 'activity_lottery_winning_record').result() - 1


@pytest.mark.parametrize('activity_id', [None, normal_activity_id, max_activity_id, min_activity_id],
                         ids=['None', 'normal', 'max', 'min'])
def test_v1_activity_lottery_myPrize(login, activity_id):
    ''' 我的奖品'''
    base_url = login.host + "/v1/activity/lottery/myPrize"
