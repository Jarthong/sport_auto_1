...
# 在真实环境才会调用
def test_v1_user__guest_openid_weibo(login, state):
    '''微博平台登录回调，需要特殊处理result=2114(用户取消或禁止使用第三方平台账号登录)的结果'''
    base_url = login.host + "/v1/user/_guest/openid/weibo"
    params = {'state': state, 'chanelId': 0}
