...
def test_v1_user__guest_register(login, accountType, country, operateType):
    ''' 用户注册'''
    base_url = login.host + "/v1/user/_guest/register"
    params = {'account': GlobalVar.GVar['user_phone'], 'loginPwd': GlobalVar.GVar['password'],
    ...
    result = r.json()
    print(result)
    assert result['result'] in ['4002', '1025', '9']
