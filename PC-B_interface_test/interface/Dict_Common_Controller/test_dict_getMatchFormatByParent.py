...
import pytest
import requests
#dict_type in(25,26,27)
parentValue = list(GetData(sql='select distinct(parent_value) from dict_common where dict_type in(25,26,27) order by parent_value asc').results())+[None]
@pytest.mark.parametrize('parentValue', parentValue)
def test_dict_getmatchformatbyparent(login, parentValue):
    ''' 获取赛制下面的具体赛制信息'''
    base_url = login.host + "/dict/getMatchFormatByParent"
